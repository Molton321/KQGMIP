...
La batería (\texttt{uv run pytest}, $275$ casos) cubre: regresión $k=2$
...
exigente de cada red.}
\end{table}

\begin{table}[H]
\centering\footnotesize
\caption{Metaheurísticas vs óptimo (KGeoMIP $=\dk^\star$ en esta tabla).}
...
\midrule
N10A & 0,04   & 0,05--0,06 & 0,20  & 0,20--0,28 \\
N15B & 1,88   & 1,66--1,77 & 4,40  & 4,46--4,64 \\
N20A & 2,23   & 0,06--0,11 & 3,18  & 0,07--0,13 \\
N22A & 28,04  & 0,14--0,26 & 17,08 & 0,11--0,22 \\
N25A & 120,55 & 0,09--0,17 & 87,90 & 0,11--0,23 \\
\bottomrule
\end{tabular}
\label{tab:time}
\par\smallskip
{\footnotesize Medido en una máquina de 16~GB. A $n=25$ la preparación de KGeoMIP
es el coste dominante ($\approx 121$~s para los cuatro $k$); KQNodes resuelve los
cuatro en $\approx 88$~s.}
\end{table}
...
(der.) sobre el benchmark N10A/N15A/N20A. En estas redes KGeoMIP es algo más rápido
que KQNodes; a $n\ge 22$ la relación se invierte (Tabla~\ref{tab:time}).}
\label{fig:scal}
\end{figure}
...
\begin{itemize}
  ...
  $\sim\!3\times$ más lento que KGeoMIP por la búsqueda Queyranne, pero a $n=22$ y
  $n=25$ la relación se invierte: KQNodes resuelve el subsistema completo en
  $\approx 17$\,s y $\approx 88$\,s frente a $\approx 28$\,s y $\approx 121$\,s de
  ...
  iguala el óptimo en toda la tabla; el \emph{recocido} se le aproxima salvo en
...
\emph{cut pool} geométrico aporta diversidad de cortes y eleva la tasa de acierto
exacto en las redes donde el óptimo es comprobable. La recomendación operativa es
ejecutar ambas y tomar el mínimo.
...
por la docente (\texttt{.core/core\_00}). El subcomando \texttt{external} comprueba
dos cosas y mide los resultados:
\begin{itemize}
...
\begin{itemize}
  ...
  cara), KGeoMIP resuelve $k=2..5$ en $\approx 121$~s y KQNodes en $\approx 88$~s.
