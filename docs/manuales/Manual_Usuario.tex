...
{\small\itshape Guía de instalación y uso para usuarios finales}\\[2cm]
{\small \today}
\vfill
\end{titlepage}
...
\begin{itemize}
  \item \textbf{Sí puede:} analizar sistemas pequeños y medianos
  ($n \le 15$–$20$ nodos) con las estrategias exactas y rápidas KGeoMIP/KQNodes,
  para cualquier $k\in\{2,3,4,5\}$.
  \item \textbf{Con cuidado:} sistemas grandes ($n \approx 25$) solo con la
  estrategia de \emph{clustering}, que \emph{propone} una división rápida pero no
  calcula la pérdida exacta (requiere demasiada memoria).
  \item \textbf{No puede:} garantizar la partición óptima global en sistemas
  grandes (usa heurísticas), ni manejar $n \gg 25$ en un equipo común.
\end{itemize}
...
Se incluye un video demostrativo (formato MP4, 8–15 min) que cubre la instalación
desde cero, la configuración y un ejemplo completo de ejecución e interpretación.

\begin{center}
\fbox{\parbox{0.8\textwidth}{\centering\vspace{4mm}
\textbf{Video tutorial:} \href{run:./video/tutorial_KQGMIP.mp4}{\texttt{video/tutorial\_KQGMIP.mp4}}\\[1mm]
\small (Enlace alternativo en línea: \emph{pendiente de publicación})\\[4mm]
}}
\end{center}

\noindent Contenido del video: (1) instalación de \texttt{uv} y
\texttt{uv sync}; (2) edición de parámetros en \texttt{main.py}; (3) ejecución con
distintos valores de $k$; (4) interpretación de la salida; (5) generación de
figuras.
...
El análisis individual se configura en \texttt{main.py} y se lanza con
\texttt{exec.py}. Los parámetros principales están al inicio de \texttt{main.py}:
...
k = 3                         # Numero de particiones (k >= 2)

STRATEGY_NAME = "kgeomip"     # kgeomip | kqnodes | clustering | exhaustive
PAGE = "A"                    # Pagina de red (A, B, C, ...)
\end{lstlisting}
...
\begin{enumerate}
  \item \textbf{Una $3$-partición de un sistema pequeño:} ponga
  \texttt{STRATEGY\_NAME="kgeomip"}, \texttt{k=3}, y ejecute \texttt{uv run exec.py}.
  \item \textbf{Comparar $k=2,3,4$:} cambie \texttt{k} y vuelva a ejecutar; anote
  la pérdida de cada uno (crecerá con $k$).
  \item \textbf{Comparar estrategias:} repita con
  \texttt{STRATEGY\_NAME="kqnodes"} y con \texttt{"clustering"} sobre el mismo
  sistema.
\end{enumerate}
...
\begin{tabular}{@{}llll@{}}
\toprule
Parámetro & Dónde & Valores & Por defecto \\
\midrule
\texttt{k} & main.py / \texttt{IIT\_K} & entero $\ge 2$ (2–5) & 3 \\
\texttt{STRATEGY\_NAME} & main.py & kgeomip, kqnodes, clustering, exhaustive, genetic, annealing, tabu & kgeomip \\
\texttt{method} & main.py / \texttt{IIT\_METHOD} & spectral, kmeans (solo clustering) & spectral \\
\texttt{PAGE} & main.py & A, B, C, \dots & A \\
\texttt{initial\_state} & main.py & cadena de $0$/$1$ de longitud $n$ & --- \\
...
\begin{itemize}
  \item \textbf{Individual} (\texttt{uv run exec.py}): un sistema, configurado en
  \texttt{main.py}.
  \item \textbf{Por lotes} (\texttt{uv run exec.py --batch}): procesa una rejilla
  desde Excel, controlado por variables de entorno:
\begin{lstlisting}[style=sh]
IIT_STRATEGY=kqnodes IIT_K=4 IIT_METHOD=spectral uv run exec.py --batch
\end{lstlisting}
  \item \textbf{Exacto} (\texttt{STRATEGY\_NAME="exhaustive"}): \emph{ground truth}
  por enumeración; úselo solo en sistemas pequeños ($n \le 6$).
\end{itemize}

\subsection{Opciones de salida y visualización}
\begin{lstlisting}[style=sh]
uv run scripts/run_benchmark.py        # rejilla -> CSV/Excel en data/results/
uv run scripts/run_benchmark.py --no-meta   # sin metaheuristicas (mas rapido)
...
\label{sec:webui}
Para quien prefiera no tocar \texttt{main.py}, el proyecto incluye una
...
\begin{enumerate}
  ...
  diagrama \textbf{interactivo} de la partición. Si existe la rejilla de
...
\begin{enumerate}
  \item En \texttt{main.py}: \texttt{initial\_state="1111"},
  \texttt{conditions="1111"}, \texttt{purview="1111"}, \texttt{mechanism="1111"},
  \texttt{k=3}, \texttt{STRATEGY\_NAME="kgeomip"}, \texttt{PAGE="A"}.
  \item Ejecutar \texttt{uv run exec.py}.
...
\subsection{Ejemplo avanzado (rejilla completa)}
\begin{lstlisting}[style=sh]
uv run scripts/run_benchmark.py        # genera la rejilla n x k
uv run scripts/validate_correctness.py # valida contra el exacto
...
\texttt{uv run exec.py} & Análisis individual (config.\ en \texttt{main.py}) \\
\texttt{uv run exec.py -{}-batch} & Análisis por lotes (Excel) \\
\texttt{uv run pytest} & Ejecutar los tests \\
\texttt{uv run scripts/run\_benchmark.py} & Rejilla de resultados \\
...
\begin{itemize}
  ...
  \item \textbf{Salida:} resumen por consola; rejillas en
