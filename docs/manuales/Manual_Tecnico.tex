...
La \textbf{Teoría de la Información Integrada} (IIT) cuantifica cuánta información
genera un sistema \emph{como un todo} por encima de la suma de sus partes. El
...
\emph{ground truth} para $n$ pequeño) y un \textbf{baseline} de \emph{clustering}
espectral/KMeans determinista.
...
\begin{itemize}
  ...
  oráculo, corrigiendo además un defecto histórico de QNodes.
  \item \textbf{Calidad:} las dos estrategias núcleo obtienen pérdidas
  $\dk$ \emph{órdenes de magnitud menores} que el baseline de clustering
  (p.\,ej.\ $0{,}47$ vs.\ $4{,}3$ en N10A, $k=2$).
  \item \textbf{Eficiencia:} vectorización NumPy ($6$–$9\times$ en la ruta
  caliente), tensores \texttt{float32} (el sistema $n=25$ cabe en RAM) y
  paralelismo por procesos (PCD) con \emph{speedup} real medido.
\end{itemize}

\subsection{Limitaciones y recomendaciones de uso}
KGeoMIP y KQNodes tienen techo práctico en $n \approx 15$–$20$ por la
marginalización $\bigO(2^n)$ y la búsqueda $\bigO(n^3)$. Para $n=25$ solo el
baseline de clustering \emph{propone} una partición en tiempo razonable, sin
puntuarla con $\dk$ (requiere el subsistema completo en memoria). Se recomienda:
KGeoMIP para análisis exactos/rápidos hasta $n\!\approx\!15$; KQNodes cuando se
busca robustez submodular; clustering solo como referencia exploratoria a gran
escala.
...
Se adopta la definición \textbf{estricta} (doc.\ oficial §2.1): los $k$ bloques
...
una bipartición (un bloque vacío sería ``inerte''), garantizando que la rejilla
...
\begin{equation}
  ...
  \label{eq:deltak}
\end{equation}
Bajo independencia condicional del repertorio de efecto, la EMD se reduce a la
suma de diferencias absolutas componente a componente (EMD-efecto
...
exacto en biparticiones sobre el núcleo verificado; Queyranne es exacto para
funciones submodulares simétricas en $\bigO(n^3)$), y (ii) la elección voraz
...
\node[box] (entry) {exec.py\\main / main\_batch};
\node[box,right=14mm of entry] (mgr) {Manager\\load\_network};
\node[data,above=8mm of mgr] (csv) {TPM\\(.csv)};
\node[core,right=14mm of mgr] (sia) {SIA\\(Template)};
\node[core,above right=6mm and 12mm of sia] (kgeo) {KGeoMIP};
\node[core,right=12mm of sia] (kqn) {KQNodes};
\node[core,below right=6mm and 12mm of sia] (clus) {Clustering};
\node[box,below=10mm of sia] (sys) {System\\$\,+\,$NCube};
\node[box,below=8mm of kqn] (kref) {greedy\_k\_partition\\+ delta\_k (EMD)};
\node[box,below=8mm of kgeo] (ct) {CostTable\,$T$};
\node[data,below=22mm of kqn] (out) {results\\(.csv/.png)};
...
% Jerarquía (izquierda) y modelo de dominio (derecha), con coordenadas explícitas.
...
\begin{algorithm}[H]
\caption{\textsc{CostTable} — tabla de costos de transición $T$ (una vez)}
\label{alg:cost}
\begin{algorithmic}[1]
\Require matriz plana por nodo $X$, estado inicial $s_0$, antípoda $s_1 = 1-s_0$
\State $\mathit{paths}[0] \gets \{s_0\}$
\For{$\mathit{level} = 1 \ldots n$}
  \For{cada $s$ en $\mathit{paths}[\mathit{level}-1]$}
    \For{cada bit $i$ con $s_i \neq (s_1)_i$}
      \State $s' \gets s$ con el bit $i$ volteado hacia $s_1$
      \State $d_H \gets \textsc{Hamming}(s_0,s')$;\quad $\gamma \gets 2^{-d_H}$
      \State $\mathit{cost} \gets |X[:,\mathrm{int}(s_0)] - X[:,\mathrm{int}(s')]|$ \Comment{gather vectorizado}
      \If{$d_H>1$} acumular costos de vecinos de un bit \EndIf
      \State $T[(s_0,s')] \gets \gamma \cdot \mathit{cost}$
    \EndFor
  \EndFor
\EndFor
...
conocidos, congelados en los tests de triaje).
...
\begin{itemize}
  ...
  \item \textbf{CostTable} — matriz contigua $(n, 2^m)$ para el \emph{gather}
  vectorizado de columnas; \texttt{transition\_table} mapea $(s_0,s')\!\to\!$ vector
  de costo.
...
\subsection{Evaluación de particiones}
Una candidata se puntúa con \texttt{delta\_k} (Ec.~\eqref{eq:deltak}): se construye
el subsistema particionado \texttt{System.k\_partition}$(\mathcal{P})$, se obtiene
su distribución marginal y se mide la EMD-efecto analítica
(Ec.~\eqref{eq:emd}) contra el repertorio base $P$.

\subsection{Optimizaciones implementadas}
\begin{itemize}
  \item \textbf{Tabla $T$ una vez} y reutilizada para todo $k$.
  \item \textbf{Vectorización NumPy:} \emph{gather} de columnas y acumulación
  vectorial en \texttt{CostTable} (eliminado el formateo de cadenas por estado);
  conjuntos de Python en la ruta caliente de \texttt{bipartition}.
  \item \textbf{Memoización} de \texttt{bipartition}/\texttt{marginalize}.
  \item \textbf{\texttt{float32}} en los tensores (mitad de memoria).
  ...
  \item \textbf{Numba (opcional):} \texttt{batch\_effect\_emd} con umbral; el
  perfilado mostró que la EMD es $<1\%$ del costo (dominado por la marginalización
  NumPy), por lo que Numba se reserva para lotes grandes.
\end{itemize}
...
(\texttt{src/funcs/metaheuristic.py}) que heredan de \texttt{SIA} y se puntúan con
la \emph{misma} pérdida $\dk$, de modo que sus resultados son directamente
comparables con KGeoMIP/KQNodes y con el óptimo exacto.
...
\begin{itemize}
  \item \textbf{Algoritmo genético (\texttt{genetic\_search}).} Población de
  soluciones reparadas; selección por torneo, cruce uniforme por átomo, mutación
  por reetiquetado con probabilidad $p_m$ y \emph{elitismo} del mejor individuo.
  \item \textbf{Recocido simulado (\texttt{simulated\_annealing\_search}).} Toma
  $\dk$ como energía; en cada paso reetiqueta un átomo y acepta el vecino con
  probabilidad de Boltzmann $\exp(-\Delta\dk/T)$, con enfriamiento geométrico
  $T \leftarrow \alpha T$.
  \item \textbf{Búsqueda tabú (\texttt{tabu\_search}).} Explora un vecindario de
  reetiquetados, escoge el mejor vecino \emph{no tabú} (con criterio de aspiración
...
\caption{Complejidad temporal y espacial por componente.}
\begin{tabular}{@{}llll@{}}
\toprule
Componente & Temporal & Espacial & Cuello de botella \\
\midrule
\texttt{marginalize} & $\bigO(2^{m})$ & $\bigO(2^{m})$ & \texttt{np.mean} (L2) \\
\texttt{CostTable} (build) & $\bigO(m\,2^{m})$ & $\bigO(2^{m})$ & BFS por niveles \\
\emph{cut pool} (KGeoMIP) & $\bigO(n)$ cortes & $\bigO(n)$ & --- \\
$\dk$ (una candidata) & $\bigO(2^{n})$ & $\bigO(2^{n})$ & marginalización \\
\textsc{Greedy} ($k$ bloques) & $\bigO(k^{2} n\, 2^{n})$ & $\bigO(2^{n})$ & evaluaciones $\dk$ \\
QNodes (Queyranne) & $\bigO(n^{3} 2^{n})$ & $\bigO(2^{n})$ & marginalización \\
Exacto (Stirling) & $\bigO(S(2n,k)\,2^{n})$ & $\bigO(2^{n})$ & enumeración \\
\bottomrule
\end{tabular}
\label{tab:complejidad}
\end{table}

\paragraph{Temporal.} KGeoMIP $\approx \bigO(m\,2^{m} + k^{2} n\, 2^{n})$;
KQNodes $\approx \bigO(n^{3} 2^{n})$ (la búsqueda submodular domina). Frente a la
fuerza bruta $\bigO(S(2n,k)\,2^{n})$, ambas heurísticas reducen el factor
combinatorio $S(2n,k)$ a uno polinómico en $n$ y $k$.

\paragraph{Espacial.} El término dominante es el tensor del subsistema y las
distribuciones, $\bigO(2^{n})$. En \texttt{float32} esto es la mitad que en
\texttt{float64}: para $n=25$, $\approx 3{,}35$ GB en lugar de $6{,}7$ GB, lo que
hace al sistema cargable en RAM.

\paragraph{Análisis de casos.} El \textbf{mejor caso} ocurre cuando el primer
corte ya aísla la masa de probabilidad (pérdida casi nula, p.\,ej.\ N15A con
$\dk=0{,}027$). El \textbf{peor caso} aparece cuando ningún corte del pool reduce
$\dk$ y deben probarse todos los pares (bloque, corte) en los $k-1$ niveles; el
costo crece con $k$ y con la densidad de la TPM. La marginalización
$\bigO(2^{m})$ es el cuello empírico: $87$ ms a $m{=}20$ y $1{,}45$ s a $m{=}24$.
...
\begin{itemize}
  ...
  \item \texttt{System.k\_partition(partition) -> System} — aplica la
  reconstrucción factorizada de una \texttt{KPartition} validada.
...
def delta_k(subsystem, partition, baseline_distribution=None):
    ...
    partitioned = subsystem.k_partition(partition).marginal_distribution()
...
\begin{table}[H]
\centering\footnotesize
\caption{Dependencias principales y su uso.}
\begin{tabular}{@{}lll@{}}
\toprule
...
pandas & 3.0.3 & E/S de TPM y rejilla de resultados \\
more\_itertools & --- & particiones de Stirling (exacto) \\
matplotlib & --- & figuras y visualización de particiones \\
pyphi & --- & validación cruzada ($6$–$10$ nodos) \\
joblib / psutil & --- & PCD (procesos), medición \\
numba (opc.) & 0.65.1 & kernel EMD por lotes (umbral) \\
pyemd (opc.) & 2.0.0 & EMD causal (métrica Hamming) de comprobación \\
plotly (opc.) & 6.8.0 & figuras interactivas (extra \texttt{viz}) \\
streamlit (opc.) & 1.58.0 & interfaz web (extra \texttt{web}) \\
\bottomrule
\end{tabular}
\end{table}
...
La batería (\texttt{uv run pytest}, $206$ casos) cubre: regresión $k=2$
...
(cargar$\to$estrategia$\to$\texttt{Solution} y \emph{roundtrip} generar/cargar),
EMD causal opcional (\texttt{importorskip}), figuras interactivas, registro headless
...
little-endian), generadas con semilla $73$. La rejilla de evaluación sigue una
estrategia \textbf{híbrida honesta}: $n\in\{10,15\}$ con las \emph{siete}
estrategias (KGeoMIP, KQNodes, Clustering~$\times2$, GA/SA/Tabú) y $\dk$ real;
$n=20$ solo con KGeoMIP (las demás del núcleo se vuelven impracticables, ver
abajo); y $n=25$ solo con clustering (que \emph{propone} la partición pero deja
$\dk$ en blanco por memoria, §\ref{sec:limites}). En todos los casos $k\in\{2,3,4\}$.
El consolidado reproducible vive en \texttt{benchmark\_results\_FINAL.csv}.
...
\begin{itemize}
  ...
  \item \textbf{Speedup} y \textbf{pendiente de escalabilidad} (log--log).
\end{itemize}

\subsection{Tabla de resultados}
\begin{table}[H]
\centering\footnotesize
\caption{Pérdida $\dk$ y tiempo por estrategia (rejilla FINAL).}
\begin{tabular}{@{}llrrr@{}}
\toprule
Red & $k$ & KGeoMIP $\dk$ & KQNodes $\dk$ & Clustering $\dk$ (mejor) \\
\midrule
N10A & 2 & 0,4697 & 0,4697 & 4,3438 \\
N10A & 3 & 0,9424 & 0,9424 & 4,8594 \\
N10A & 4 & 1,4229 & 1,4229 & 4,8594 \\
N15A & 2 & 0,0268 & 0,0268 & 0,5274 \\
N15A & 3 & 0,0538 & 0,0539 & 0,8752 \\
N15A & 4 & 0,0809 & 0,0810 & 1,0762 \\
N20A & 2 & 0,4993 & --- & --- \\
N20A & 3 & 0,9987 & --- & --- \\
N20A & 4 & 1,4982 & --- & --- \\
\bottomrule
...
\label{tab:loss}
\par\smallskip
{\footnotesize En N20A solo KGeoMIP es practicable ($\sim\!82$ s/$k$); «---» marca
estrategias no ejecutadas por inviabilidad temporal (KQNodes) o de memoria
(clustering sin $\dk$).}
\end{table}

\begin{table}[H]
\centering\footnotesize
\caption{Metaheurísticas vs óptimo (KGeoMIP $=\dk^\star$ en esta rejilla).}
\begin{tabular}{@{}llrrrr@{}}
\toprule
Red & $k$ & Óptimo & Tabú & Genética & Recocido \\
\midrule
N10A & 2 & 0,4697 & 0,4697 & 0,4697 & 0,4727 \\
N10A & 3 & 0,9424 & 0,9424 & 2,3809 & 0,9639 \\
N10A & 4 & 1,4229 & 1,4229 & 3,8848 & 1,4365 \\
N15A & 2 & 0,0268 & 0,0268 & 0,0268 & 0,0271 \\
N15A & 3 & 0,0538 & 0,0538 & 0,5561 & 0,0540 \\
N15A & 4 & 0,0809 & 0,0809 & 0,9198 & 0,8346 \\
\bottomrule
\end{tabular}
\label{tab:meta}
\end{table}
...
\begin{table}[H]
\centering\footnotesize
\caption{Tiempo de ejecución (s), perfilador apagado.}
\begin{tabular}{@{}llrr@{}}
\toprule
Red & $k$ & KGeoMIP (s) & KQNodes (s) \\
\midrule
N10A & 2 & 0,051 & 0,181 \\
N10A & 3 & 0,048 & 0,193 \\
N10A & 4 & 0,055 & 0,176 \\
...
(der.). KGeoMIP es consistentemente más rápido que KQNodes y es la única
estrategia núcleo que alcanza $n=20$ ($\sim\!82$\,s).}
\label{fig:scal}
\end{figure}
...
(Streamlit, \texttt{app/streamlit\_app.py}) que permite, sin línea de comandos:
...
interactiva. El registro headless \texttt{src/funcs/runner.py}
...
\begin{itemize}
  ...
  \item \textbf{Núcleo $\gg$ baseline:} las estrategias núcleo superan al
  clustering por $\sim\!10\times$ en N10A y $\sim\!20\times$ en N15A; el clustering
  optimiza una métrica de grafo, no $\dk$.
  \item \textbf{Costo de KQNodes:} $\sim\!3\times$ más lento que KGeoMIP por la
  búsqueda Queyranne $\bigO(n^3)$, sin ganancia de calidad en esta rejilla.
  \item \textbf{Monotonía:} $\dk$ crece con $k$ (Fig.~\ref{fig:lossk}), coherente
  con que más bloques destruyen más información.
  \item \textbf{KGeoMIP escala a $n=20$:} computa $\dk$ en $\sim\!82$\,s/$k$
  (Tabla~\ref{tab:time}), un punto empírico que valida la cota $\bigO(2^n)$
  (de $n=15$ a $n=20$, $2^5=32\times$ más estados $\Rightarrow$ salto de
  $\sim\!1{,}6$\,s a $\sim\!82$\,s, factor $\sim\!50\times$, dominado por la
  marginalización). KQNodes no termina en tiempo de sesión ahí (Queyranne
  $\bigO(n^3 2^n)$).
  \item \textbf{Metaheurísticas (Tabla~\ref{tab:meta}):} la \emph{búsqueda tabú}
  iguala el óptimo en toda la rejilla; el \emph{recocido} se le aproxima salvo en
...
\begin{itemize}
  ...
  GeoMIP) coincide $21/21$; KGeoMIP iguala el óptimo exacto y KQNodes acierta
  $9/10$ (corrige el defecto N3B de QNodes: $0{,}46875$ vs $0{,}5$).
...
\begin{itemize}
  \item \textbf{Techo de escala:} KGeoMIP/KQNodes son prácticos hasta
  $n\approx 15$–$20$ (marginalización $\bigO(2^n)$, Queyranne $\bigO(n^3)$). A
  $n=20$, KGeoMIP$_{k=2}\approx 52$–$84$ s y KQNodes$\approx 348$ s.
  \item \textbf{$n=25$:} solo el clustering produce una partición (en $<5$ s),
  pero \emph{no} se puntúa con $\dk$ porque requiere el \texttt{System} completo en
  memoria (OOM). Las filas N25A reportan partición sin pérdida, documentado
  honestamente.
...
\begin{itemize}
  \item Marginalización \emph{local} $\bigO(2^{\dim})$ por bloque para subir el
  techo de $n$.
  \item Refinamiento con \emph{look-ahead} o \emph{beam search} para acercarse al
  óptimo global manteniendo costo polinómico.
  \item Baseline de metaheurísticas (GA/SA/Tabú), diferido (Fase 5B).
...
\paragraph{Reflexión crítica.} La IA aceleró tareas mecánicas (vectorización,
\emph{boilerplate}, redacción) y propuso refactors útiles; sin embargo, varias
...
criterio de validación contra oráculo y \emph{microbenchmarks} aislados fue
imprescindible para no aceptar mejoras ilusorias. La comprensión profunda del
algoritmo y la responsabilidad de las decisiones permanecen en el equipo.
