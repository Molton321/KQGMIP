...
Subcommands (each forwards its remaining options to the underlying module):
    ...
    uv run scripts/validate.py external [--rows N | --verify-partitions ...]
        Cross-validation of original project (.core/core_00): byte-identical
        TPM samples, reproduction of the original GeoMIP results, and re-evaluation
        of every stored partition of the results workbook with the official delta_k.
"""
...
def main() -> None:
    ...
    if len(sys.argv) < 2 or sys.argv[1] not in MODULES:
        print(__doc__)
        raise SystemExit(
            0 if len(sys.argv) >= 2 and sys.argv[1] in ("-h", "--help") else 2
        )
