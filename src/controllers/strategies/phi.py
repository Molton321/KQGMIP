...
class Phi(SIA):
    ...
    def apply_strategy(
        ...
        try:
            ...
            raise ImportError("pyphi no está instalado. Instálalo con: pip install pyphi") from err
        ...
        purview_idxs = tuple(
            i for i, (b, c) in enumerate(zip(purview, condition, strict=False))
            if b == STR_ONE and c == STR_ONE
        )
        mechanism_idxs = tuple(
            i for i, (b, c) in enumerate(zip(mechanism, condition, strict=False))
            if b == STR_ONE and c == STR_ONE
        )
