...
El marco de referencia \textbf{GeoMIP}~\cite{geomip} (y su variante submodular QNodes)
resuelve únicamente \textbf{biparticiones}
...
\begin{itemize}
	...
	\item \textbf{Eficiencia:} tabla de costos vectorizada e indexada por el entero
	      ...
	      diccionario, creciente con $m$) y marginal local; n-cubos compactos en \texttt{uint8} (TPM 0/1)
	      (pico medido $\sim$6~GB a $n=25$, cabe en 16~GB) y paralelismo por procesos (PCD).
\end{itemize}
...
El marco geométrico de partida, incluida la tabla de costos de transición, la
recurrencia por niveles de Hamming y el factor $\gamma=2^{-d_H}$, es el publicado en
GeoMIP~\cite{geomip}; allí se formula y valida \emph{solo} para $k=2$. La clave de
la extensión (doc.\ §2.2) es que \textbf{una $k$-partición equivale a
...
doble: (i) cada corte hereda la garantía de su método base (GeoMIP~\cite{geomip} es óptimo
...
\begin{itemize}
	\item \textbf{NCube} — tensor $(2,\dots,2)$ \texttt{uint8}/\texttt{float32} (según la TPM) por columna de la
...
\begin{itemize}
	...
	\item \textbf{TPM en \texttt{uint8}:} las TPMs deterministas 0/1 se cargan como
	      ...
	      \texttt{float32}); los n-cubos son \emph{vistas} de ese buffer (sin copia) y la
	      aritmética (marginales, tabla de costos) castea a \texttt{float32} donde se
	      necesita, de modo que el resultado es bit-idéntico. Las TPMs continuas conservan
...
de la tabla a $n=25$ tampoco: un solo worker alcanza $7{,}9$~GB de pico, así que
...
\paragraph{Espacial.} Dos términos conviven en $n=25$: los
n-cubos del subsistema (vistas \texttt{uint8} de la TPM, $0{,}84$~GB) y la
tabla $T$ ($2^m \times n$, $3{,}36$~GB en \texttt{float32} cuando $m=n$, el
término dominante). El pico medido de KGeoMIP en N25A es de
$\sim$6~GB, dentro de una máquina de 16~GB; KQNodes, que no construye $T$,
alcanza $\sim$1~GB.
...
\begin{itemize}
	...
	\item \textbf{Metaheurísticas (Tabla~\ref{tab:meta}):} la \emph{búsqueda tabú}
	      ...
	      pues el espacio $k^{2n}$ excede su presupuesto por defecto ($60$ individuos
	      $\times\,80$ generaciones). Es el comportamiento esperado de una metaheurística sin reinicio y
...
$7{,}9$~GB. En una máquina de 16~GB eso obliga a ejecutar las filas grandes en serie,
...
\subsubsection{Frente al artículo publicado GeoMIP (oráculo PyPhi)}
\label{sec:val-geomip}
El marco que este proyecto extiende fue publicado como \emph{GeoMIP}~\cite{geomip}, que
aporta un conjunto de resultados de referencia validados contra \textbf{PyPhi} (el patrón
de oro exhaustivo) en \texttt{data/results/Pruebas\_Metodo2.xlsx}. Como las TPMs de
referencia son las mismas redes $N\{n\}$ de este repositorio (idénticas byte a byte en
N3A--N15B), la comparación es directa. Se contrastó \texttt{KGeoMIP} ($k=2$, el régimen
que el artículo formula) contra la columna PyPhi sobre los \textbf{371 subsistemas} de las
ocho hojas ($n\in\{3,4,5,6,8,10,15\}$), con las mismas métricas de la Tabla~3 del
artículo (tasa de acierto, error relativo de $\varphi$ y distancia estructural):
\begin{itemize}
	\item \textbf{Reproducción del método base:} \texttt{KGeoMIP} reproduce la columna
	      GeoMIP del artículo con \textbf{0 discrepancias} (verificado fila a fila en la hoja
	      15A); por construcción es su Método 2 (programación dinámica).
	\item \textbf{Acierto vs PyPhi:} $100{,}0\%$ ($88/88$) sobre los casos con
	      $\varphi$ no trivial ($\varphi\ge 0{,}01$) y $93{,}5\%$ en bruto a 4 decimales.
	\item \textbf{Error relativo de $\varphi$:} máximo $0{,}16\%$ (umbral ``Excelente''
	      del artículo: $<1\%$).
	\item \textbf{Distancia estructural:} $0$ en $344/347$ casos; los $3$ restantes
	      ($\le 0{,}67$) son \emph{óptimos alternativos} con $\dk$ idéntica, fenómeno que el
	      propio artículo reporta (hasta $0{,}5$).
\end{itemize}
Con los umbrales de la Tabla~3 del artículo (Excelente: acierto $>90\%$, error
$<1\%$), \texttt{KGeoMIP} queda en nivel \textbf{Excelente}. La brecha del acierto en
bruto se concentra en subsistemas con $\varphi\approx 0$ (redes grandes, reducibles),
donde el propio GeoMIP del artículo también difiere de PyPhi y donde su Ec.~(6) prescribe
medir el error en valor absoluto; medido así, el acuerdo es total.
...
\begin{thebibliography}{99}
	\bibitem{geomip} J. Díaz-Arancibia, L. E. Guerrero, J. Arango-López,
		L. F. Castillo, A. Bustamante-Mora. \emph{GeoMIP: A Geometric-Topological and
		Dynamic Programming Framework for Enhanced Computational Tractability of Minimum
		Information Partition in Integrated Information Theory}. Applied Sciences,
		16(2):809, 2026. \texttt{https://doi.org/10.3390/app16020809}.
